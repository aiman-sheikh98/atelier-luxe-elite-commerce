      ...
      <Link to={`/product/${product.id}`} className="block">
        <div className="relative overflow-hidden">
          ...
          <div className="absolute top-2 left-2 flex flex-col gap-1">
            {product.new && (
              <span className="bg-luxury-light text-white px-3 py-1 text-xs font-medium">
                New
              </span>
            )}
            {product.bestseller && (
              <span className="bg-luxury-dark text-white px-3 py-1 text-xs font-medium">
                Bestseller
              </span>
            )}
          </div>
          ...
          >
            <Heart 
              size={18} 
              className={isInWishlist ? "fill-luxury text-luxury" : ""}
            />
          </Button>
        </div>
